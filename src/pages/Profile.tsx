...
const Profile: React.FC = () => {
  ...
  const [userData, setUserData] = useState<any>(null); // Use the hook inside the component
  const navigate = useNavigate();

  const fetchUserData = async (userId: string) => {
    try {
      const docRef = doc(db, "users", userId);  // Access the user's document by their UID
  ...
  useEffect(() => {
    if (currentUser) {
      fetchUserData(currentUser.uid); // Fetch user data when currentUser changes
    }
  }, [currentUser]);
    ...
    <>
      ...
      <div className="overflow-hidden rounded-sm border border-stroke bg-white shadow-default dark:border-strokedark dark:bg-boxdark">
        ...
        <div className="px-4 pb-6 text-center lg:pb-8 xl:pb-11.5">
          <div className="relative z-30 mx-auto -mt-22 h-30 w-30 sm:h-44 sm:w-44 rounded-full bg-white/20 p-1 backdrop-blur sm:p-3">
            <div className="object-cover w-full h-full">
              <img src={UserOne} alt="profile" className='object-cover w-full h-full' />
              {/* <label
                htmlFor="profile"
